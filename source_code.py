...
def ok_error_command():
    ...
    Label(screen, text="").pack()
    Title_Label = Label(
        screen, text="Title of Window Error", font=('Arial', 14))
    Title_Label.pack()
    ...
    global compile_btn
    compile_btn = Button(
        screen, width=50, text='Compile Your Error', command=compile_ok_error)
...
def compile_ok_error():
    ...
    Label(screen, text="Finished Compiling, your error is in the Errors folder",
          font=('Arial', 12)).pack()
    Label(screen, text="").pack()
    Button(screen, text="Preview Error", command=preview_ok_error).pack()
    compile_btn.destroy()
    Label(screen, text="").pack()
    Button(screen, text="Compile Another Error!",
           width=30, command=refresh).pack()
    Label(screen, text="").pack()
    Button(screen, text="By Just Another Coder!",
           width=25, command=subscribe).pack()
...
def ct_error_command():
    ...
    Text1_Label = Label(
        screen, text="Text in Window Error", font=('Arial', 14))
    Text1_Label.pack()
    ...
    global compile1_btn
    compile1_btn = Button(
        screen, width=50, text='Compile Your Error', command=compile_ct_error)
...
def alert_error_command():
    ...
    global compile2_btn
    compile2_btn = Button(
        screen, width=50, text='Compile Your Error', command=compile_alert_error)
...
def main_screen():
    ...
    global Label1
    Label1 = Label(
        text="Welcome to Error Simulator! Created by Just Another Coder", font=('Arial', 14))
    Label1.pack()

    Label(text="").pack()
    global Label2
    Label2 = Label(
        text="Choose what type of error you want to simulate below by clicking on it:", font=('Arial', 14))
    Label2.pack()

    Label(text="").pack()
    global ok_err
    ok_error = PhotoImage(file=r"assets/ok_error.png")
    ok_err = Button(screen, text='Ok Error', image=ok_error,
                    compound=BOTTOM, command=ok_error_command)
    ok_err.pack(side=TOP)

    Label(text="").pack()
    global ct_error
    caution_error = PhotoImage(file=r"assets/caution_error.png")
    ct_error = Button(screen, text='Caution Error', image=caution_error,
                      compound=BOTTOM, command=ct_error_command)
    ct_error.pack(side=TOP)

    Label(text="").pack()
    global virus_err
    virus_error = PhotoImage(file=r"assets/virus_error.png")
    virus_err = Button(screen, text='Alert Error', image=virus_error,
                       compound=BOTTOM, command=alert_error_command)
